...
def home(request):
    ...
    projects = None
    review1 = None
    project_id = None  # Initialize project_id as None
    ...
    if form.is_valid():
        ...
        # Retrieve the highest review rating for each project and sort projects by that rating
        if projects:
            projects = projects.annotate(highest_rating=Max('reviews__rating')).order_by('-highest_rating')  # Sort projects by highest rating from their reviews

    # Handle review submission
    if request.method == "POST" and 'project_id' in request.POST:
        project_id = request.POST.get('project_id')  # Only get project_id if it exists in POST data
        ...
        if project_id:
            ...
            if form.is_valid():
                ...
                review.rating = rating  # Set the rating from the form input
                review.save()  # Save the review

                # Optionally, re-filter the projects after saving the review
                projects = Project.objects.filter(id=project_id)
                projects = projects.annotate(highest_rating=Max('reviews__rating')).order_by('-highest_rating')  # Sort projects again

    # Retrieve reviews for the sorted projects
    if projects:
        review1 = Review.objects.filter(project__in=projects)
...
def dashboard_view(request):
    # Fetch all projects and their associated reviews (ratings)
    # projects_with_reviews = Project.objects.all().prefetch_related('reviews')
    projects_with_reviews = Project.objects.filter(reviews__isnull=False).distinct()
    ...
    for project in projects_with_reviews:
        ...
        project_data.append({
            ...
            'reviews': [review.rating for review in reviews]  # List of ratings for this project
        })
